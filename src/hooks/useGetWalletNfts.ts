...
import { getSaladRabbitContract } from 'utils/contractHelpers'
import makeBatchRequest from 'utils/makeBatchRequest'

const saladRabbitsContract = getSaladRabbitContract()
...
const useGetWalletNfts = () => {
  ...
  useEffect(() => {
    const fetchNfts = async () => {
      try {
        const balanceOf = await saladRabbitsContract.methods.balanceOf(account).call()
        ...
        if (balanceOf > 0) {
          ...
          const getTokenIdAndBunnyId = async (index: number) => {
            try {
              const { tokenOfOwnerByIndex, getBunnyId, tokenURI } = saladRabbitsContract.methods

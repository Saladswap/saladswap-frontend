...
import { Button, IconButton, useModal, AddIcon, Image } from '@saladswap/uikit'
...
import OldSauceTitle from './OldSauceTitle'
...
const PoolCard: React.FC<HarvestProps> = ({ pool }) => {
  ...
  const isOldSauce = stakingTokenName === QuoteToken.SAUCE
    ...
    <Card isActive={isCardActive} isFinished={isFinished && sousId !== 0}>
      ...
      <div style={{ padding: '24px' }}>
        <CardTitle isFinished={isFinished && sousId !== 0}>
          {isOldSauce && '[OLD]'} {tokenName} {TranslateString(348, 'Pool')}
        </CardTitle>
        <div style={{ marginBottom: '8px', display: 'flex', alignItems: 'center' }}>
          <div style={{ flex: 1 }}>
            <Image src={`/images/tokens/${image || tokenName}.png`} width={64} height={64} alt={tokenName} />
          </div>
          {account && harvest && !isOldSauce && (
            <HarvestButton
        ...
        {!isOldSauce ? (
          ...
          <OldSauceTitle hasBalance={accountHasStakedBalance} />
        )}
        <Label isFinished={isFinished && sousId !== 0} text={TranslateString(330, `${tokenName} earned`)} />
        <StyledCardActions>
          {!account && <UnlockButton />}
          {account &&
            (needsApproval && !isOldSauce ? (
              <div style={{ flex: 1 }}>

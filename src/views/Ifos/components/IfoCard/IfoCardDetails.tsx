...
import { Text, LinkExternal, Link, Box, CardFooter, Button, ChevronDownIcon, ChevronUpIcon } from '@saladswap/uikit'
...
const IfoCardDetails: React.FC<IfoCardDetailsProps> = ({ ifo, publicIfoData }) => {
  ...
  const { description, saladToBurn, projectSiteUrl, launchDate, launchTime, saleAmount, raiseAmount } = ifo
      ...
      {isOpen && (
        <>
          ...
          <Box mb="24px">
            ...
            <Item>
              <Display>{TranslateString(586, 'SALAD to burn (USD)')}</Display>
              <Text>{saladToBurn}</Text>
            </Item>
            <Item>

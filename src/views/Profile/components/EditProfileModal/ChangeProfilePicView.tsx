...
import { Button, InjectedModalProps, Skeleton, Text } from '@saladswap/uikit'
...
import { useSaladRabbits, useProfile as useProfileContract } from 'hooks/useContract'
import { getSaladProfileAddress, getSaladRabbitsAddress } from 'utils/addressHelpers'
...
const ChangeProfilePicPage: React.FC<ChangeProfilePicPageProps> = ({ onDismiss }) => {
  ...
  const {
    ...
    onApprove: () => {
      return saladRabbitsContract.methods.approve(getSaladProfileAddress(), tokenId).send({ from: account })
    },
    onConfirm: () => {
      if (!profile.isActive) {
        return profileContract.methods.reactivateProfile(getSaladRabbitsAddress(), tokenId).send({ from: account })
      }

      return profileContract.methods.updateProfile(getSaladRabbitsAddress(), tokenId).send({ from: account })
    },
    onSuccess: async () => {
      // Re-fetch profile
    ...
    <>
      ...
      {!isLoading && walletNfts.length === 0 && (
        <>
          ...
          <Text as="p" color="textSubtle" mb="24px">
            {TranslateString(999, 'Make sure you have a Salad Collectible in your wallet and try again!')}
          </Text>
        </>
      )}

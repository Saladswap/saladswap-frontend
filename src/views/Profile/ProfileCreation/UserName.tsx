...
const minimumSaladToRegister = new BigNumber(REGISTER_COST).multipliedBy(new BigNumber(10).pow(18))
...
const UserName: React.FC = () => {
  ...
  const { teamId, tokenId, userName, actions, minimumSaladRequired, allowance } = useProfileCreation()
  ...
  const hasMinimumSaladRequired = useHasSaladBalance(minimumSaladToRegister)
  const [onPresentConfirmProfileCreation] = useModal(
    <ConfirmProfileCreationModal
      ...
      account={account}
      teamId={teamId}
      minimumSaladRequired={minimumSaladRequired}
      allowance={allowance}
    />,
    false,
  )
    ...
    <>
      ...
      {!hasMinimumSaladRequired && (
        <Text color="failure" mt="16px">
          {TranslateString(1098, `A minimum of ${REGISTER_COST} SALAD is required`, { num: REGISTER_COST })}
        </Text>
      )}
    </>
  )
